...
class ViewController: UIViewController {
    ...
    private let connectionString = "HostName=SZIoT.azure-devices.net;DeviceId=myiOSdevice2;SharedAccessKey=+j/GCv9Cn5rzd+yMiinwWTggc6k4XWC5x7WZHrDHuq4="
    ...
    override func viewDidLoad() {
        ...
        timer = Timer.scheduledTimer(timeInterval: 0.5, target: self, selector: #selector(ViewController.updateVar), userInfo: nil, repeats: true)
    ...
    @objc func updateVar(){
        ...
        let accelerData = motionManager.accelerometerData
        let accelX = accelerData?.acceleration.x
        self.accelTagX.text = String(format: "%.05f", accelX!)
        let accelY = accelerData?.acceleration.y
        self.accelTagY.text = String(format: "%.05f", accelY!)
        let accelZ = accelerData?.acceleration.x
        self.accelTagZ.text = String(format: "%.05f", accelZ!)
                // print("Accelerometer Z: \(accelZ!)")
          
        let gyroData = motionManager.gyroData
        let gyroX = gyroData?.rotationRate.x
        self.gyroTagX.text = String(format: "%.05f", gyroX!)
        let gyroY = gyroData?.rotationRate.y
        self.gyroTagY.text = String(format: "%.05f", gyroY!)
        let gyroZ = gyroData?.rotationRate.x
        self.gyroTagZ.text = String(format: "%.05f", gyroZ!)
                

        var data : [String : String] = ["accx": accelTagX.text! ,"accy":accelTagY.text! ,"accz":accelTagZ.text!,"gyrox":gyroTagX.text!,"gyroy":gyroTagY.text!,"gyroz":gyroTagZ.text!]
        telm = data.description
